...
import { slackAdapter } from '../controllers/slack'

export default (controller: Botkit) => {

    if (controller.adapter !== slackAdapter ) return

    controller.ready(async () => {

        // let bot:any = await controller.spawn({
        //     incoming_webhook: {
        //         url: process.env.SLACK_WEBHOOK_CONSCIOUSNESS
        //     }
        // })

        // bot.sendWebhook({
        //     text: "I Am Awoken",
        //     channel: "#bot-consciousness"
        // });
    ...
    controller.on('direct_message', async(bot: any, message) => {
        await bot.reply(message,'I heard a private message');
    });

    controller.hears('dm me', 'message', async(bot: any, message) => {
    ...
    controller.on('app_mention', async(bot, message) => {
        console.log(message)
        await bot.reply(message, `I heard a app mention from <@${ message.user }> at "${ message.incoming_message.timestamp }"`);
    });
    ...
    controller.hears('ephemeral', 'message,direct_message', async(bot: any, message) => {
        await bot.replyEphemeral(message,'This is an ephemeral reply sent using bot.replyEphemeral()!');
    });

    controller.hears('threaded', 'message,direct_message', async(bot: any, message) => {
    ...
    controller.on('slash_command', async(bot: any, message) => {
    ...
    controller.on('interactive_message', async (bot: any, message) => {
    ...
    controller.on('dialog_submission', async (bot: any, message) => {
